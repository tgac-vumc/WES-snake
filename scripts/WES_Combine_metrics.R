...
readStat<-snakemake@input[["readStat"]]
...
if (length(readLines(insert_size_metrics)) > 6){
...
Total <- read.delim(readStat, header=F)[1,1]
Aligned <- read.delim(gsub('.all', '.aligned', readStat), header=F)[1,1]
Q37 <- read.delim(gsub('.all', '.q37', readStat), header=F)[1,1]
Q1 <- read.delim(gsub('.all', '.q1', readStat), header=F)[1,1]
Unique <- read.delim(gsub('.all', '.unique', readStat), header=F)[1,1]


readmetrics <- data.frame(TOTALREADS=Total, ALIGNED_READS=Aligned, Q1_READS=Q1, Q37_READS=Q37, UNIQUE_READS=Unique)

print(readmetrics)

if (!is.na(insert_metrics)) {
all<-cbind(readmetrics, metrics,alignmetrics, insert_metrics)
} else{
all<-cbind(readmetrics, metrics,alignmetrics)
}
all$SAMPLE<-sample
...
columns <- c("SAMPLE", "TOTALREADS", "TOTAL_READS","ALIGNED_READS", "Q1_READS", "Q37_READS", "UNIQUE_READS",
		"PF_UNIQUE_READS","PCT_PF_UQ_READS","PF_UQ_READS_ALIGNED", "PCT_PF_UQ_READS_ALIGNED", "PCT_OFF_BAIT",
...
columns <- columns[columns%in%colnames(all)]
